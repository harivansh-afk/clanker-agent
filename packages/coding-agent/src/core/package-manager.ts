...
function collectFiles(
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (isDir) {
        files.push(
          ...collectFiles(fullPath, filePattern, skipNodeModules, ig, root),
        );
...
function collectAutoSkillEntries(
  dir: string,
  includeRootFiles = true,
): string[] {
...
function matchesAnyPattern(
  ...
  return patterns.some((pattern) => {
    ...
    return (
      minimatch(parentRel!, pattern) ||
      minimatch(parentName!, pattern) ||
      minimatch(parentDir!, pattern)
    );
  });
}
...
function isEnabledByOverrides(
  ...
  if (
    ...
    matchesAnyExactPattern(filePath, forceIncludes, baseDir)
  ) {
    enabled = true;
  }
  if (
    forceExcludes.length > 0 &&
    matchesAnyExactPattern(filePath, forceExcludes, baseDir)
  ) {
    enabled = false;
  }
  return enabled;
...
function applyPatterns(
  ...
  if (includes.length === 0) {
    ...
    result = allPaths.filter((filePath) =>
      matchesAnyPattern(filePath, includes, baseDir),
    );
  }
  ...
  if (excludes.length > 0) {
    result = result.filter(
      (filePath) => !matchesAnyPattern(filePath, excludes, baseDir),
    );
  }
...
export class DefaultPackageManager implements PackageManager {
  ...
  addSourceToSettings(source: string, options?: { local?: boolean }): boolean {
    ...
    const normalizedSource = this.normalizePackageSourceForSettings(
      source,
      scope,
    );
    const exists = currentPackages.some((existing) =>
      this.packageSourcesMatch(existing, source, scope),
    );
    if (exists) {
      return false;
    }
  ...
  removeSourceFromSettings(
    ...
    const nextPackages = currentPackages.filter(
      (existing) => !this.packageSourcesMatch(existing, source, scope),
    );
  ...
  async update(source?: string): Promise<void> {
    ...
    for (const pkg of projectSettings.packages ?? []) {
      ...
      if (
        identity &&
        this.getPackageIdentity(sourceStr, "project") !== identity
      )
        continue;
  ...
  private async updateSourceForScope(
    source: string,
    scope: SourceScope,
  ): Promise<void> {
  ...
  private async resolvePackageSources(
    ...
    for (const { pkg, scope } of sources) {
      ...
      if (parsed.type === "local") {
        const baseDir = this.getBaseDirForScope(scope);
        this.resolveLocalExtensionSource(
          parsed,
          accumulator,
          filter,
          metadata,
      ...
      if (parsed.type === "npm") {
        ...
        const needsInstall =
          ...
          (await this.npmNeedsUpdate(parsed, installedPath));
        if (needsInstall) {
          const installed = await installMissing();
          if (!installed) continue;
        }
        metadata.baseDir = installedPath;
        this.collectPackageResources(
          installedPath,
          accumulator,
          filter,
          metadata,
  ...
  private resolveLocalExtensionSource(
    ...
    try {
      ...
      if (stats.isDirectory()) {
        ...
        const resources = this.collectPackageResources(
          resolved,
          accumulator,
          filter,
          metadata,
  ...
  private packageSourcesMatch(
    ...
    const left = this.getSourceMatchKeyForSettings(
      this.getPackageSourceString(existing),
      scope,
    );
  ...
  private dedupePackages(
    ...
    for (const entry of packages) {
      const sourceStr =
        typeof entry.pkg === "string" ? entry.pkg : entry.pkg.source;
  ...
  private async updateGit(
    ...
    await this.runCommand("git", ["fetch", "--prune", "origin"], {
      cwd: targetDir,
    });

    // Reset to tracking branch. Fall back to origin/HEAD when no upstream is configured.
    try {
      await this.runCommand("git", ["reset", "--hard", "@{upstream}"], {
        cwd: targetDir,
      });
    } catch {
      ...
      await this.runCommand("git", ["reset", "--hard", "origin/HEAD"], {
        cwd: targetDir,
      });
    }
  ...
  private async refreshTemporaryGitSource(
    source: GitSource,
    sourceStr: string,
  ): Promise<void> {
    if (isOfflineModeEnabled()) {
  ...
  private resolveLocalEntries(
    ...
    const resolvedPlain = plain.map((p) =>
      this.resolvePathFromBase(p, baseDir),
    );
