...
function loadNamedContextFileFromDir(
  dir: string,
  filename: string,
): { path: string; content: string } | null {
  const filePath = join(dir, filename);
...
export class DefaultResourceLoader implements ResourceLoader {
  ...
  async reload(): Promise<void> {
    ...
    const themePaths = this.noThemes
      ...
      : this.mergePaths(
          [...enabledThemes, ...cliEnabledThemes],
  ...
  private applyExtensionMetadata(
    ...
    for (const resourcePath of resourcePaths) {
      ...
      if (
        this.pathMetadata.has(normalizedResourcePath) ||
  ...
  private addDefaultMetadataForPath(filePath: string): void {
    ...
    if (
      this.pathMetadata.has(normalizedPath) ||
      this.pathMetadata.has(filePath)
    ) {
      return;
    }
