...
export class ToolExecutionComponent extends Container {
  ...
  constructor(
    ...
    if (
      toolName === "bash" ||
      (toolDefinition && !this.shouldUseBuiltInRenderer())
    ) {
  ...
  private shouldUseBuiltInRenderer(): boolean {
    ...
    const hasCustomRenderers =
      this.toolDefinition?.renderCall || this.toolDefinition?.renderResult;
  ...
  private rebuildWriteHighlightCacheFull(
    rawPath: string | null,
    fileContent: string,
  ): void {
  ...
  private updateWriteHighlightCacheIncremental(): void {
    ...
    cache.highlightedLines[lastIndex] = this.highlightSingleLine(
    ...
    for (let i = 1; i < segments.length; i++) {
      ...
      cache.highlightedLines.push(
        this.highlightSingleLine(segments[i], cache.lang),
      );
    }
  ...
  private updateDisplay(): void {
    ...
    if (useBuiltInRenderer) {
      ...
      if (this.toolDefinition.renderCall) {
        try {
          const callComponent = this.toolDefinition.renderCall(
            this.args,
            theme,
          );
    ...
    if (!useBuiltInRenderer && this.toolDefinition) {
      this.hideComponent =
        !customRendererHasContent && this.imageComponents.length === 0;
    }
  }
    ...
    if (this.toolName === "read") {
      ...
      const firstChangedLine =
        (this.editDiffPreview && "firstChangedLine" in this.editDiffPreview
          ...
          : undefined) ||
        (this.result && !this.result.isError
          ? this.result.details?.firstChangedLine
          : undefined);
      if (firstChangedLine) {
