  ...
  try {
    switch (options.command) {
      ...
      case "list": {
        ...
        const formatPackage = (
          pkg: (typeof globalPackages)[number],
          scope: "user" | "project",
        ) => {
...
export async function main(args: string[]) {
  ...
  const isInteractive =
    !isGatewayCommand && !parsed.print && parsed.mode === undefined;
  ...
  if (isGatewayCommand) {
    ...
    const persistPrimarySession = sessionManager
    ...
    while (true) {
      ...
      const { session, modelFallbackMessage } = await createGatewaySession(
      ...
      const daemonOptions: DaemonModeOptions = {
        ...
        createSession: async (sessionKey) => {
          ...
          const gatewaySessionOptions: CreateAgentSessionOptions = {
            ...
            settingsManager,
            resourceLoader: gatewayResourceLoader,
            sessionManager: createGatewaySessionManager(
              cwd,
              sessionKey,
              gatewaySessionRoot,
            ),
          };
          const { session: gatewaySession } = await createAgentSession(
