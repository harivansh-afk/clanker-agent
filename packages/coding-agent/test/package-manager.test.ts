  ...
  describe("resolve", () => {
    it("should return no package-sourced paths when no sources configured", async () => {
      ...
      expect(
        result.skills.every(
          (r) =>
            r.metadata.source === "auto" && r.metadata.origin === "top-level",
        ),
      ).toBe(true);
    });
    ...
    it("should resolve local extension paths from settings", async () => {
      ...
      const result = await packageManager.resolve();
      expect(
        result.extensions.some((r) => r.path === extPath && r.enabled),
      ).toBe(true);
    });
...
Content`,
      ...
      expect(result.skills.some((r) => r.path === skillFile && r.enabled)).toBe(
        true,
      );
    });
    ...
    it("should auto-discover user prompts with overrides", async () => {
      ...
      const result = await packageManager.resolve();
      expect(
        result.prompts.some((r) => r.path === promptPath && !r.enabled),
      ).toBe(true);
    });
    ...
    it("should resolve directory with package.json companion.extensions in extensions setting", async () => {
      ...
      writeFileSync(
        join(pkgDir, "extensions", "clip.ts"),
        "export default function() {}",
      );
      writeFileSync(
        join(pkgDir, "extensions", "cost.ts"),
        "export default function() {}",
      );
      writeFileSync(
        join(pkgDir, "extensions", "helper.ts"),
        "export const x = 1;",
      ); // Not in manifest, shouldn't be loaded
      ...
      expect(
        result.extensions.some(
          (r) => r.path === join(pkgDir, "extensions", "cost.ts") && r.enabled,
        ),
      ).toBe(true);

      // Should NOT find helper.ts (not declared in manifest)
      expect(result.extensions.some((r) => r.path.endsWith("helper.ts"))).toBe(
        false,
      );
    });
  });
  ...
  describe(".agents/skills auto-discovery", () => {
    it("should scan .agents/skills from cwd up to git repo root", async () => {
      ...
      mkdirSync(join(tempDir, ".agents", "skills", "above-repo"), {
        recursive: true,
      });
      writeFileSync(
        aboveRepoSkill,
      ...
      mkdirSync(join(repoRoot, ".agents", "skills", "repo-root"), {
        recursive: true,
      });
      writeFileSync(
        repoRootSkill,
      ...
      mkdirSync(join(repoRoot, "packages", ".agents", "skills", "nested"), {
        recursive: true,
      });
      writeFileSync(
        nestedSkill,
      ...
      expect(
        result.skills.some((r) => r.path === repoRootSkill && r.enabled),
      ).toBe(true);
      expect(
        result.skills.some((r) => r.path === nestedSkill && r.enabled),
      ).toBe(true);
    ...
    it("should scan .agents/skills up to filesystem root when not in a git repo", async () => {
      ...
      mkdirSync(join(nonRepoRoot, ".agents", "skills", "root"), {
        recursive: true,
      });
      writeFileSync(rootSkill, "---\nname: root\ndescription: root\n---\n");

      const middleSkill = join(
        nonRepoRoot,
        "a",
        ".agents",
        "skills",
      ...
      mkdirSync(join(nonRepoRoot, "a", ".agents", "skills", "middle"), {
        recursive: true,
      });
      writeFileSync(
        middleSkill,
      ...
      const result = await pm.resolve();
      expect(result.skills.some((r) => r.path === rootSkill && r.enabled)).toBe(
        true,
      );
      expect(
  ...
  describe("ignore files", () => {
    it("should respect .gitignore in skill directories", async () => {
      ...
      const result = await packageManager.resolve();
      expect(
        result.skills.some((r) => r.path.includes("good-skill") && r.enabled),
      ).toBe(true);
      expect(
        result.skills.some((r) => r.path.includes("venv") && r.enabled),
      ).toBe(false);
    });
    ...
    it("should not apply parent .gitignore to .companion auto-discovery", async () => {
      ...
      const result = await packageManager.resolve();
      expect(result.skills.some((r) => r.path === skillPath && r.enabled)).toBe(
        true,
      );
    });
  });
  ...
  describe("resolveExtensionSources", () => {
    ...
    it("should handle directories with companion manifest", async () => {
      ...
      expect(
        result.extensions.some(
          (r) => r.path === join(pkgDir, "src", "index.ts") && r.enabled,
        ),
      ).toBe(true);
      // Skills with SKILL.md are returned as file paths
      expect(
        result.skills.some(
          (r) =>
            r.path === join(pkgDir, "skills", "my-skill", "SKILL.md") &&
            r.enabled,
        ),
      ).toBe(true);
    });
    ...
    it("should handle directories with auto-discovery layout", async () => {
      ...
      writeFileSync(
        join(pkgDir, "extensions", "main.ts"),
        "export default function() {}",
      );
      ...
      const result = await packageManager.resolveExtensionSources([pkgDir]);
      expect(
        result.extensions.some((r) => r.path.endsWith("main.ts") && r.enabled),
      ).toBe(true);
      expect(
        result.themes.some((r) => r.path.endsWith("dark.json") && r.enabled),
      ).toBe(true);
    });
  });
  ...
  describe("source parsing", () => {
    it("should emit progress events on install attempt", async () => {
      ...
      expect(
        events.some((e) => e.type === "start" && e.action === "install"),
      ).toBe(true);
    ...
    it("should parse package source types from docs examples", () => {
      expect(
        (packageManager as any).parseSource("npm:@scope/pkg@1.2.3").type,
      ).toBe("npm");
      expect((packageManager as any).parseSource("npm:pkg").type).toBe("npm");

      expect(
        (packageManager as any).parseSource("git:github.com/user/repo@v1").type,
      ).toBe("git");
      expect(
        (packageManager as any).parseSource("https://github.com/user/repo@v1")
          .type,
      ).toBe("git");
      expect(
        (packageManager as any).parseSource("git:git@github.com:user/repo@v1")
          .type,
      ).toBe("git");
      expect(
        (packageManager as any).parseSource("ssh://git@github.com/user/repo@v1")
          .type,
      ).toBe("git");

      expect(
        (packageManager as any).parseSource("/absolute/path/to/package").type,
      ).toBe("local");
      expect(
        (packageManager as any).parseSource("./relative/path/to/package").type,
      ).toBe("local");
      expect(
        (packageManager as any).parseSource("../relative/path/to/package").type,
      ).toBe("local");
    ...
    it("should never parse dot-relative paths as git", () => {
      const dotSlash = (packageManager as any).parseSource(
        "./packages/agent-timers",
      );
      expect(dotSlash.type).toBe("local");
      expect(dotSlash.path).toBe("./packages/agent-timers");

      const dotDotSlash = (packageManager as any).parseSource(
        "../packages/agent-timers",
      );
  ...
  describe("settings source normalization", () => {
    it("should store global local packages relative to agent settings base", () => {
      ...
      const added = packageManager.addSourceToSettings(
        "./packages/local-global-pkg",
      );
    ...
    it("should store project local packages relative to .companion settings base", () => {
      ...
      writeFileSync(
        join(projectPkgDir, "extensions", "index.ts"),
        "export default function() {}",
      );

      const added = packageManager.addSourceToSettings("./project-local-pkg", {
        local: true,
      });
      expect(added).toBe(true);
    ...
    it("should remove local package entries using equivalent path forms", () => {
      ...
      expect(settingsManager.getGlobalSettings().packages ?? []).toHaveLength(
        0,
      );
    });
  });
  ...
  describe("HTTPS git URL parsing (old behavior)", () => {
    ...
    it("should generate correct package identity for protocol and git:-prefixed URLs", async () => {
      const identity1 = (packageManager as any).getPackageIdentity(
        "https://github.com/user/repo",
      );
      const identity2 = (packageManager as any).getPackageIdentity(
        "https://github.com/user/repo@v1.0.0",
      );
      const identity3 = (packageManager as any).getPackageIdentity(
        "git:github.com/user/repo",
      );
      const identity4 = (packageManager as any).getPackageIdentity(
        "https://github.com/user/repo.git",
      );
    ...
    it("should deduplicate git URLs with different supported formats", async () => {
      ...
      mkdirSync(join(pkgDir, "extensions"), { recursive: true });
      writeFileSync(
        join(pkgDir, "extensions", "test.ts"),
        "export default function() {}",
      );
      ...
      const id1 = (packageManager as any).getPackageIdentity(
        "https://github.com/user/repo",
      );
      const id2 = (packageManager as any).getPackageIdentity(
        "git:github.com/user/repo",
      );
      const id3 = (packageManager as any).getPackageIdentity(
        "https://github.com/user/repo.git",
      );
    ...
    it("should handle HTTPS URLs with refs in resolve", async () => {
      ...
      const parsed2 = (packageManager as any).parseSource(
  ...
  describe("pattern filtering in top-level arrays", () => {
    it("should exclude extensions with ! pattern", async () => {
      ...
      expect(result.extensions.some((r) => isDisabled(r, "remove.ts"))).toBe(
        true,
      );
    });
    ...
    it("should filter prompts with exclusion pattern", async () => {
      ...
      expect(result.prompts.some((r) => isDisabled(r, "explain.md"))).toBe(
        true,
      );
    });
    ...
    it("should filter skills with exclusion pattern", async () => {
      ...
      const result = await packageManager.resolve();
      expect(
        result.skills.some((r) => isEnabled(r, "good-skill", "includes")),
      ).toBe(true);
      expect(
        result.skills.some((r) => isDisabled(r, "bad-skill", "includes")),
      ).toBe(true);
    });
  ...
  describe("pattern filtering in companion manifest", () => {
    it("should support glob patterns in manifest extensions", async () => {
      ...
      mkdirSync(join(pkgDir, "extensions"), { recursive: true });
      mkdirSync(join(pkgDir, "node_modules/dep/extensions"), {
        recursive: true,
      });
      writeFileSync(
        join(pkgDir, "extensions", "local.ts"),
        "export default function() {}",
      );
      writeFileSync(
      ...
      expect(result.extensions.some((r) => isEnabled(r, "remote.ts"))).toBe(
        true,
      );
      expect(result.extensions.some((r) => r.path.endsWith("skip.ts"))).toBe(
        false,
      );
    });
    ...
    it("should support glob patterns in manifest skills", async () => {
      ...
      expect(result.skills.some((r) => r.path.includes("bad-skill"))).toBe(
        false,
      );
    });
  });
  ...
  describe("pattern filtering in package filters", () => {
    it("should apply user filters on top of manifest filters (not replace)", async () => {
      ...
      mkdirSync(join(pkgDir, "extensions"), { recursive: true });
      writeFileSync(
        join(pkgDir, "extensions", "foo.ts"),
        "export default function() {}",
      );
      writeFileSync(
        join(pkgDir, "extensions", "bar.ts"),
        "export default function() {}",
      );
      writeFileSync(
        join(pkgDir, "extensions", "baz.ts"),
        "export default function() {}",
      );
      writeFileSync(
        join(pkgDir, "package.json"),
      ...
      expect(result.extensions.some((r) => r.path.endsWith("baz.ts"))).toBe(
        false,
      );
    });
    ...
    it("should combine include and exclude patterns", async () => {
      ...
      mkdirSync(join(pkgDir, "extensions"), { recursive: true });
      writeFileSync(
        join(pkgDir, "extensions", "alpha.ts"),
        "export default function() {}",
      );
      writeFileSync(
        join(pkgDir, "extensions", "beta.ts"),
        "export default function() {}",
      );
      writeFileSync(
        join(pkgDir, "extensions", "gamma.ts"),
        "export default function() {}",
      );
      ...
      const result = await packageManager.resolve();
      expect(result.extensions.some((r) => isEnabled(r, "alpha.ts"))).toBe(
        true,
      );
      expect(result.extensions.some((r) => isDisabled(r, "beta.ts"))).toBe(
        true,
      );
      expect(result.extensions.some((r) => isDisabled(r, "gamma.ts"))).toBe(
        true,
      );
    });
    ...
    it("should work with direct paths (no patterns)", async () => {
      ...
      mkdirSync(join(pkgDir, "extensions"), { recursive: true });
      writeFileSync(
        join(pkgDir, "extensions", "one.ts"),
        "export default function() {}",
      );
      writeFileSync(
        join(pkgDir, "extensions", "two.ts"),
        "export default function() {}",
      );
  ...
  describe("force-include patterns", () => {
    it("should force-include extensions with + pattern after exclusion", async () => {
      ...
      writeFileSync(join(extDir, "keep.ts"), "export default function() {}");
      writeFileSync(
        join(extDir, "excluded.ts"),
        "export default function() {}",
      );
      writeFileSync(
        join(extDir, "force-back.ts"),
        "export default function() {}",
      );
      ...
      const result = await packageManager.resolve();
      expect(result.extensions.some((r) => isDisabled(r, "keep.ts"))).toBe(
        true,
      );
      expect(result.extensions.some((r) => isDisabled(r, "excluded.ts"))).toBe(
        true,
      );
      expect(result.extensions.some((r) => isEnabled(r, "force-back.ts"))).toBe(
        true,
      );
    });
    ...
    it("should force-include overrides exclude in package filters", async () => {
      ...
      const result = await packageManager.resolve();
      expect(result.extensions.some((r) => isDisabled(r, "alpha.ts"))).toBe(
        true,
      );
    ...
    it("should force-include multiple resources", async () => {
      ...
      const result = await packageManager.resolve();
      expect(
        result.skills.some((r) => isEnabled(r, "skill-a", "includes")),
      ).toBe(true);
      expect(
        result.skills.some((r) => isDisabled(r, "skill-b", "includes")),
      ).toBe(true);
      expect(
        result.skills.some((r) => isEnabled(r, "skill-c", "includes")),
      ).toBe(true);
    });
    ...
    it("should handle force-include in manifest patterns", async () => {
      ...
      writeFileSync(
        join(pkgDir, "extensions", "three.ts"),
        "export default function() {}",
      );
      writeFileSync(
        join(pkgDir, "package.json"),
      ...
      expect(result.extensions.some((r) => isEnabled(r, "three.ts"))).toBe(
        true,
      );
    });
    ...
    it("should force-include themes", async () => {
      ...
      expect(result.themes.some((r) => isEnabled(r, "special.json"))).toBe(
        true,
      );
    });
  ...
  describe("package deduplication", () => {
    it("should dedupe same local package in global and project (project wins)", async () => {
      ...
      mkdirSync(join(pkgDir, "extensions"), { recursive: true });
      writeFileSync(
        join(pkgDir, "extensions", "shared.ts"),
        "export default function() {}",
      );
      ...
      const sharedPaths = result.extensions.filter((r) =>
        r.path.includes("shared-pkg"),
      );
    ...
    it("should keep both if different packages", async () => {
      ...
      writeFileSync(
        join(pkg1Dir, "extensions", "from-pkg1.ts"),
        "export default function() {}",
      );
      writeFileSync(
        join(pkg2Dir, "extensions", "from-pkg2.ts"),
        "export default function() {}",
      );
    ...
    it("should dedupe SSH and HTTPS URLs for same repo", async () => {
      ...
      const httpsIdentity = (packageManager as any).getPackageIdentity(
        httpsUrl,
      );
    ...
    it("should dedupe SSH URL with ssh:// protocol and git@ format", async () => {
      ...
      const sshProtocolIdentity = (packageManager as any).getPackageIdentity(
        sshProtocol,
      );
    ...
    it("should dedupe all supported URL formats for same repo", async () => {
      ...
      const identities = urls.map((url) =>
        (packageManager as any).getPackageIdentity(url),
      );
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
      ...
      writeFileSync(
        join(pkgDir, "extensions", "standalone.ts"),
      ...
      expect(
        result.extensions.some(
          (r) => r.path.endsWith("subagent/index.ts") && r.enabled,
        ),
      ).toBe(true);
      expect(
        result.extensions.some(
          (r) => r.path.endsWith("standalone.ts") && r.enabled,
        ),
      ).toBe(true);

      // Should NOT find agents.ts as a standalone extension
      expect(result.extensions.some((r) => r.path.endsWith("agents.ts"))).toBe(
        false,
      );
    });
    ...
    it("should respect package.json companion.extensions manifest in subdirectories", async () => {
      ...
      writeFileSync(
        join(pkgDir, "extensions", "custom", "utils.ts"),
        "export const util = 1;",
      );
      ...
      expect(
        result.extensions.some(
          (r) => r.path.endsWith("custom/main.ts") && r.enabled,
        ),
      ).toBe(true);

      // Should NOT find utils.ts (not declared in manifest)
      expect(result.extensions.some((r) => r.path.endsWith("utils.ts"))).toBe(
        false,
      );
    });
    ...
    it("should handle mixed top-level files and subdirectories", async () => {
      ...
      // Top-level extension
      writeFileSync(
        join(pkgDir, "extensions", "simple.ts"),
        "export default function(api) {}",
      );
      ...
      writeFileSync(
        join(pkgDir, "extensions", "complex", "a.ts"),
        "export const a = 1;",
      );
      writeFileSync(
        join(pkgDir, "extensions", "complex", "b.ts"),
        "export const b = 2;",
      );
      ...
      expect(
        result.extensions.some(
          (r) => r.path.endsWith("simple.ts") && r.enabled,
        ),
      ).toBe(true);
      expect(
        result.extensions.some(
          (r) => r.path.endsWith("complex/index.ts") && r.enabled,
        ),
      ).toBe(true);

      // Should NOT find helper modules
      expect(
        result.extensions.some((r) => r.path.endsWith("complex/a.ts")),
      ).toBe(false);
      expect(
        result.extensions.some((r) => r.path.endsWith("complex/b.ts")),
      ).toBe(false);
    ...
    it("should skip subdirectories without index.ts or manifest", async () => {
      ...
      writeFileSync(
        join(pkgDir, "extensions", "broken", "helper.ts"),
        "export const x = 1;",
      );
      writeFileSync(
        join(pkgDir, "extensions", "broken", "another.ts"),
        "export const y = 2;",
      );

      // Valid top-level extension
      writeFileSync(
        join(pkgDir, "extensions", "valid.ts"),
        "export default function(api) {}",
      );
      ...
      expect(
        result.extensions.some((r) => r.path.endsWith("valid.ts") && r.enabled),
      ).toBe(true);
  ...
  describe("offline mode and network timeouts", () => {
    it("should skip installing missing package sources when offline", async () => {
      ...
      const installParsedSourceSpy = vi.spyOn(
      ...
      expect(allResources.some((r) => r.metadata.origin === "package")).toBe(
        false,
      );
    ...
    it("should skip refreshing temporary git sources when offline", async () => {
      ...
      writeFileSync(
        join(installedPath, "extensions", "index.ts"),
        "export default function() {};",
      );
      ...
      const result = await packageManager.resolveExtensionSources([gitSource], {
        temporary: true,
      });
      expect(
        result.extensions.some(
          (r) => r.path.endsWith("extensions/index.ts") && r.enabled,
        ),
      ).toBe(true);
    ...
    it("should pass an AbortSignal timeout when fetching npm latest version", async () => {
      ...
      const latest = await (packageManager as any).getLatestNpmVersion(
        "example",
      );
