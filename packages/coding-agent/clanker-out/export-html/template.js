    ...
    switch (entry.type) {
      case "message": {
        ...
        if (msg.role === "toolResult") {
          ...
          return (
            labelHtml +
            `<span class="tree-role-tool">[${msg.toolName || "tool"}]</span>`
          );
        }
      ...
      case "model_change":
        return (
          labelHtml +
          `<span class="tree-muted">[model: ${entry.modelId}]</span>`
        );
      case "thinking_level_change":
        return (
          labelHtml +
          `<span class="tree-muted">[thinking: ${entry.thinkingLevel}]</span>`
        );
      default:
    ...
    const renderResultImages = () => {
      ...
      return (
        ...
        images
          .map(
            (img) =>
              `<img src="data:${img.mimeType};base64,${img.data}" class="tool-image" />`,
          )
          .join("") +
        "</div>"
      );
